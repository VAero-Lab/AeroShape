...
class VolumeCalculator:
    ...
    @staticmethod
    def compute_solid_volume_sai(X, Y, Z):
        """Compute volume via Section-Area Integration (SAI).

        Instead of applying the Divergence Theorem on linear triangles
        (which underestimates volume due to inscribed-polygon error), this
        method computes the exact cross-sectional area at each spanwise
        station using the shoelace formula, then integrates along the span
        with the trapezoidal rule.

        This eliminates the chordwise curvature approximation error that
        causes the ~4-5 % bias in ``compute_solid_volume``.  The only
        remaining error source is the spanwise trapezoidal integration,
        which is typically very small for smooth planforms.

        Parameters
        ----------
        X, Y, Z : np.ndarray
            Coordinate matrices of shape (n_sections, m_points).
            Each row is a closed airfoil profile at a spanwise station.
        ...
        """
        n_sec, m_pts = X.shape
        areas = np.zeros(n_sec)
        y_pos = np.zeros(n_sec)

        for j in range(n_sec):
            # Shoelace formula for the area of a closed polygon
            # in the local (x, z) plane of the section
            x = X[j, :]
            z = Z[j, :]
        ...
        # Trapezoidal integration along span
        volume = np.trapz(areas, y_pos)
        return abs(volume)
